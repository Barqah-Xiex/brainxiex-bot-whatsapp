...
async function message(sock, m, store) {
    ...
    if(Akun.pJob != 73) return nyarios(`Kamu bukan tukang ojek !\ndaftar kerja dulu di *${Prefix}pekerjaan*`);
    ...
    const jarak = Math.floor(Math.random() * 10);
    
    await loading.editLoading(`Jarak *${jarak}KM*`);
    
    await sleep(1000);
    
    await loading.editLoading(`*Kamu* sedang mengojek dan menempuh jarak *${jarak}KM*`);
