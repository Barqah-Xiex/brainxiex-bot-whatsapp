...
async function message(sock, m, store) {
    ...
    const {data: {Barqah:{url:link,link:yt,audio,title,thumb,error}}} = await sock.sendRequest(m).post(`${baseURL}/api/downloader/play`,{apikey,search:arg, type:'audio'}).catch(v => ({data: {v,Barqah:{error:true}}}));
    if (error||false) {
        nyarios(`Error.\n\n${data.v.message}`);
