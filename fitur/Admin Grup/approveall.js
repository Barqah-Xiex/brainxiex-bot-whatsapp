...
async function message(sock, m, store) {
    ...
    await sock.groupRequestParticipantsUpdate(id,listIDMasuk,"approve");

    nyarios(sock.cardStyle(
        'APPROVE ALL',
        `✓ semua permintaan masuk telah di approve

- Grup: ${subject}
- Jumlah: ${listIDMasuk.length} anggota
- Admin: @${nomor}

Anggota yang di approve:
${listIDMasuk.map(v => `- @${v.split('@')[0]}`).join('\n')}`,
        Nama_Bot
    ));
}
