...
module.exports = async (Baileys, config, func) => {
    ...
    const liana = (Baileys?.makeWASocket || Baileys?.default)(configConnect);

    const store = liana.signalRepository.lidMapping;
    store.lidMapping = liana.signalRepository.lidMapping;
    store.plugin = fs.cek(`./database/plugin.json`) ? jsonparse(`${fs.load(`./database/plugin.json`)}`) : {}
     ...
     */
    store.loadMessage = function (chat, id) {
        const fixFileName = (file) => `${file}`?.replace(/\//g, '__')?.replace(/:/g, '-');
        const toMsgPath = (c, i) => func.pathResolve(func.pathJoin(`./database/msg/`,fixFileName(c),fixFileName(i)));

        
        console.log("test1",toMsgPath(chat,id));
        if(!fs.cek(toMsgPath(chat,id)) && func.isLid(chat)) chat = store.getPN(chat);
        console.log("test2",toMsgPath(chat,id));
        if(!fs.cek(toMsgPath(chat,id)) && func.isPN(chat)) chat = store.getLid(chat);
        console.log("test3",toMsgPath(chat,id));

        const path = toMsgPath(chat,id);
    ...
    store.saveMessage = function (chat, id, m) {
        ...
        fs.asave(file, JSON.stringify(m));

        return true;
    }
    store.saveMessages = async function () {
        const dbdir = func.pathResolve(`./database/msg/`);
        if(!fs.cek(dbdir)) fs.mkdirSync(dbdir, {recursive: true});
    ...
    store.save = async () => {
        ...
        await fs.asave(`./database/plugin.json`, JSON.stringify(store.plugin));
        
        await store.saveMessages();
    }

    store.findLid = function (sender, type = 'lid') {
        ...
        if (type === 'lid') return senderIsLid ? sender : (mappedIsLid ? mapped : null);
        return !senderIsLid ? sender : (!mappedIsLid ? mapped : null);
    };

    store.getPNfromServer = liana.signalRepository.lidMapping.getLIDForPN;
    store.getLIDfromServer = liana.signalRepository.lidMapping.getPNForLID;

    store.getLid = (sender) => store.findLid(sender, 'lid');
    store.getPN = (sender) => store.findLid(sender, 'pn');

    store.getLidAsync = async (sender) => {
        const onStore = store.getLid(sender);
        if(!onStore) return await store.getLIDfromServer(sender);
        return onStore;
    };

    store.getPNAsync = async (sender) => {
        const onStore = store.getPN(sender);
        if(!onStore) return await store.getPNfromServer(sender);
        return onStore;
    };
    ...
    liana.ev.on('messages.upsert', async chatUpdate => {
        try {
            chatUpdate.messages.forEach(async(mek, keberapa) => {
                ...
                store.saveMessage(chat, m.key.id, m);
                
                store.contacts[sender] = {...(store.contacts[sender]||{}), name: pushName, id: sender};
                // store.contacts[sender] = 


                if(m.senderLid && m.senderJid) {
                    const prevLid = store.lid[m.senderLid];
                    const prevJid = store.lid[m.senderJid];
                    const changed = prevLid !== m.senderJid || prevJid !== m.senderLid;
                ...
                if(m.senderLid) store.lid[m.senderLid] = liana.decodeJid(await store.getPNfromServer(m.senderLid) || m.senderJid);
                if(m.senderJid) store.lid[m.senderJid] = liana.decodeJid(await store.getLIDfromServer(m.senderJid) || m.senderLid);

                console.log(m.senderLid, m.senderJid, store.lid[m.senderLid], store.lid[m.senderJid]);
