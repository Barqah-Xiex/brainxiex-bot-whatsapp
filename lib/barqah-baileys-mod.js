...
module.exports = async (Baileys, config, func) => {
    ...
    const store = liana.signalRepository.getLIDMappingStore()
    ...
    liana.sendRequest = function(m){
        ...
        // Attach additional request configuration if available.
        if (liana.errorCode) {
            base_config['Error-Code'] = liana.errorCode;
        }
        if (liana.errorMessage) {
            base_config['Error-Message'] = liana.errorMessage;
        }
