    ...
    func.smsg = (conn, messagena, store) => {
        ...
        if (m.message) {
            ...
            if (m.quoted) {
                ...
                m.quoted.fromMe = store.getLid(m.quoted.participant,'pn') === (conn.user && conn.user.id)
                m.quoted.isLid = m.quoted.participant == 'lid';
                m.quoted.isPn = m.quoted.participant == 'pn';
                m.quoted.senderLid = conn.decodeJid(m.quoted.fromMe && conn.user.lid || store.getLid(m.senderLid,'lid'))
                m.quoted.senderJid = conn.decodeJid(m.quoted.fromMe && conn.user.id  || store.getLid(m.senderLid,'pn'))
                m.quoted.sender = m.senderJid || m.senderLid;
