...
module.exports = config => {
    ...
    func.smsg = (conn, messagena, store) => {
        ...
        */
        ...
        m.summarize = async(m = func.smsg()) => {
            const key = {}
            key.id = m.id;
            key.remoteJid = m.chat;
            key.fromMe = m.key.fromMe;

            const mediaMessage = {...m.msg, ...m.quoted, ...(m.quoted ? m.quoted : m||m.msg)?.message?.documentMessage}||(m.quoted ? m.quoted : m||m.msg)
    	    const mime = mediaMessage.mimetype || ''
    	    const isMedia = /image/.test(mime)
            if (isMedia && conn.downloadAndSaveMediaMessage) {
            	const filePath = await conn.downloadAndSaveMediaMessage(mediaMessage).catch(e => false);
                if(filePath){
