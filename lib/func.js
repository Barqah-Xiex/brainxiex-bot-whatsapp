...
module.exports = config => {
    ...
    func.searchIsLidOrNot = function (input1='',input2='',type = 'lid') {
        if(type == 'lid') {
            if(func.isLid(input1)) return input1;
            if(func.isLid(input2)) return input2;
        } else {
            if(!func.isLid(input1)) return input1;
            if(!func.isLid(input2)) return input2;
        }
    ...
    func.smsg = (conn, messagena, store) => {
        ...
        if (m.key) {
            ...
            m.senderLid = conn.decodeJid(m.fromMe && conn.user.lid || ((m.participant || m.key.participant || m.key.remoteJid),(m.participantAlt || m.key.participantAlt || m.key.remoteJidAlt), 'lid') || m.chat || '')
            m.senderJid = conn.decodeJid(m.fromMe && conn.user.id ||  ((m.participant || m.key.participant || m.key.remoteJid),(m.participantAlt || m.key.participantAlt || m.key.remoteJidAlt), 'pn' ) || m.chat || '')
            ...
            m.pushName = m.pushName || store?.contacts?.[m.senderJid]?.name || (m.fromMe && m.user?.name)
        }
        if (m.message) {
