...
module.exports = config => {
    ...
    func.userAgent = function ({Nama_Bot = "UNKNOWN BOT",Nomor_Bot = NaN,Nama_Owner = "UNKNOWN",Nomor_Owner=NaN}) {
        ...
        const totalCores = os.cpus().length; // Mendapatkan jumlah core CPU
    ...
    func["smsg"] = (conn, messagena, store) => {
        ...
        if (m.key) {
            ...
            m.sender = conn.decodeJid(m.fromMe && conn.user.id || m.participantAlt|| m.participant || m.key.participant || m.chat || '')
            m.saha = conn.decodeJid(m.fromMe && conn.user.id || m.participantAlt || m.key.participantAlt || m.chat || '')
    ...
    func.load = async(link, type = "string") => {
        const Buffer = await func.meida2buffer(link);
        const data = `${Buffer}`;
