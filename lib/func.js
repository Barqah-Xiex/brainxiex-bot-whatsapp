...
module.exports = config => {
    ...
    func.smsg = (conn, messagena, store) => {
        ...
        if (m.key) {
            ...
            m.senderLid = conn.decodeJid(m.fromMe && conn.user.lid || m.isLid ? (m.participant || m.key.participant || m.key.remoteJid) : (m.participantAlt || m.key.participantAlt || m.key.remoteJidAlt) || m.chat || '')
            m.senderJid = conn.decodeJid(m.fromMe && conn.user.id || !m.isLid ? (m.participant || m.key.participant || m.key.remoteJid) : (m.participantAlt || m.key.participantAlt || m.key.remoteJidAlt) || m.chat || '')
